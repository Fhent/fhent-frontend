...
const useUnwrap = (amount: string, refetchBalance?: () => void) => {
  ...
  const handleUnWrap = useCallback(async () => {
    ...
    const einput = zamaClient
      .createEncryptedInput(addresses.zamaWEERC20, account)
      .add64(Number(amount) * 10 ** 6)
      .encrypt();
    ...
    if (refetchBalance) {
      setTimeout(async () => await refetchBalance(), 15000);
    }
  }, [
    amount,
    account,
